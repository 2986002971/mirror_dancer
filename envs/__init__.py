...
# Add the project root to Python path to handle Hydra's directory changes
...
def missing_dependencies(task, test=False):
    raise ValueError(
    ...
    if cfg.multitask:
        ...
        for fn in [
            ...
            try:
                ...
                if "xmlCtxtParseDocument" in str(e):
                    print(f"   → XML 解析冲突在 {fn.__name__}")
                pass
        if env is None:
            # 详细错误信息
            import traceback
            ...
            print("\n❌ 环境创建失败详情:")
            print(f"任务: {cfg.task}")
            print("尝试的环境类型: dmcontrol, maniskill, metaworld, myosuite")
            traceback.print_exc()
            raise ValueError(
